...
import { StatusDot, ServiceStatusBadge } from "@/components/ui/status-dot";
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto">
        {/* Header */}
        <div className="flex items-center gap-3 px-6 py-2 border-b border-[#1e2d3d] bg-[#050709] sticky top-0 z-10 text-[9px] font-mono uppercase tracking-widest text-[#374151]">
          <span className="w-4" />
          <span className="flex-1">Source</span>
          <span className="w-28 hidden md:block">Format</span>
          <span className="w-20 hidden lg:block">Transport</span>
          <span className="w-24 text-right">Events/min</span>
          <span className="w-16 text-right">Parse%</span>
          <span className="w-16 text-right">Lag</span>
          <span className="w-24 text-right hidden xl:block">Last Event</span>
        </div>
        ...
        {loading ? (
          ...
          sources.map((src) => (
            <Link
              key={src.id}
              href={`/sources/${src.id}`}
              className="flex items-center gap-3 px-6 py-3 border-b border-[#1e2d3d]/50 hover:bg-[#0d1117] transition-colors group"
            >
              ...
              <span className="text-[#64748b] text-[10px] font-mono w-28 hidden md:block truncate">{src.format.replace(/_/g, " ")}</span>
              <span className="text-[#64748b] text-[10px] font-mono w-20 hidden lg:block">{src.transport}</span>
              <span className="text-[#94a3b8] text-[10px] font-mono w-24 text-right">
              ...
              <span className="hidden xl:block w-24 text-right">
